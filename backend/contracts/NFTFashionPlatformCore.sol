...
contract NFTFashionPlatformCore is ERC721URIStorage, Ownable {
    ...
    function singleUploadDesign(
        ...
        designs[newTokenId] = Design(newTokenId, _tokenURI, msg.sender, _category, _designType, _price, 0);
    ...
    require(artists[artist].registered, "Artist is not registered.");
    require(msg.sender != address(0), "Invalid sender address.");

    uint256 newTokenId = tokenCounter;
    _mint(msg.sender, newTokenId);

    artistMembershipNFTs[artist].push(newTokenId);

    tokenCounter++;

    emit MembershipNFTMinted(artist, msg.sender, newTokenId);
}
 function canViewPremiumNFTs(address artist, address viewer) public view returns (bool) {
        uint256[] memory ownedNFTs = artistMembershipNFTs[artist];
        for (uint256 i = 0; i < ownedNFTs.length; i++) {
            if (ownerOf(ownedNFTs[i]) == viewer) {
                return true;
            }
        }
        return false;
